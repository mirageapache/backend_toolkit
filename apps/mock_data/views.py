...
class CustomMockDataView(APIView):
    ...
    def get(self, request, schema_id=None, *args, **kwargs):
        ...
        generator = CustomGenerator(schema_definition=schema_obj.schema, locale=locale)
        # 生成資料
        data = generator.generate_multi(count=count)
        # 分頁處理
        paginator = self.pagination_class()
    ...
    def post(self, request, *args, **kwargs):
        ...
        # 直接把拿到的規則餵給生成器
        generator = CustomGenerator(schema_definition=schema_definition, locale=locale)
        data = generator.generate_multi(count=count)
        # 處理分頁
        paginator = self.pagination_class()
        page = paginator.paginate_queryset(data, request, view=self)
